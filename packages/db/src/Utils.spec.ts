...
import { q } from './Query';
import { simplifyBinaryTree, decomposeBinaryTree, renameField } from './Utils';
...
describe('Utils', () => {
	...
	it('renameField', async () => {
		...
		const aa = renameField(a, new Map([
			[q.field('foo', 'bar'), q.field('foo2')]
		]));
		expect(aa).to.not.equal(a);
		expect(renameField(a, new Map())).to.equal(a);
		...
		const bb = renameField(b, new Map([
			[q.field('foo', 'bar'), q.field('foo2')]
		]));
		expect(bb).to.not.equal(b);
		expect(renameField(b, new Map())).to.equal(b);
		...
		const cc = renameField(c, new Map([
			[q.field('foo', 'bar'), q.field('foo2')]
		]));
		expect(cc).to.not.equal(c);
		expect(renameField(c, new Map())).to.equal(c);
