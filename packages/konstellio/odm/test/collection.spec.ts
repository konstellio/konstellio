...
describe('Collection', () => {
	const postCatSchema: Schema = {
		handle: 'PostCategory',
		fields: [{ handle: 'id', type: 'string' }, { handle: 'title', type: 'string', required: true }],
		indexes: [],
	};
	...
	it('initialize', async () => {
		...
		expect(
			() => new Collection<ProductFields, ProductIndexes, ProductInputs>(db, ['fr', 'en'], productSchema)
		).to.not.throw();
		expect(() => new Collection(db, ['fr', 'en'], postSchema)).to.not.throw();
		expect(() => new Collection(db, [], postSchema)).to.not.throw();
		expect(
			() =>
				new Collection<ProductFields, ProductIndexes, ProductInputs>(db, ['fr', 'en'], productSchema, ['_type'])
		).to.not.throw();
	});
	...
	it('findById', async () => {
		...
		const Product = new Collection<ProductFields, ProductIndexes, ProductInputs>(db, ['fr', 'en'], productSchema, [
			'_type',
		]);
	...
	it('findMany', async () => {
		...
		const res = await Post.findMany({
			fields: ['id'],
			condition: q.or(q.eq('id', 'post-a'), q.eq('id', 'post-b')),
			sort: [q.sort('id')],
			offset: 1,
		});
