...
export function copy(fsSource: FileSystem, source: string, fsDestination: FileSystem, destination: string) {
	...
	const mapDestination = new Transform({
		...
		transform(entry, encoding, done) {
				...
				entry[0],
				join(destination, relative(source, entry[0]))
					.split(sep)
					.join('/'),
				entry[1],
			]);
	...
	const copyFiles =
		fsSource instanceof FileSystemPool
	...
	return lstree(fsSource, source)
		.pipe(mapDestination)
		.pipe(createDirectories)
		.pipe(copyFiles);
}
