...
export function mergeAST(documents: DocumentNode[]): DocumentNode {
	...
	return {
		kind: concat.kind,
		definitions: concat.definitions.reduce(
			(definitions, node) => {
				if (isTypeDefinitionNode(node)) {
					...
					if (extensions) {
						if (node.kind === Kind.OBJECT_TYPE_DEFINITION) {
							definitions.push({
								...node,
								interfaces: (node.interfaces || []).concat(
									extensions.reduce((interfaces, ext) => {
										interfaces.push(...ext.interfaces);
										return interfaces;
									}, [])
								),
								directives: mergeDirectives(
									(node.directives || []).concat(
										extensions.reduce((directives, ext) => {
											directives.push(...ext.directives);
											return directives;
										}, [])
									)
								),
								fields: (node.fields || []).concat(
									extensions.reduce((fields, ext) => {
							...
							definitions.push({
								...
								values: (node.values || []).concat(
									extensions.reduce((values, ext) => {
										values.push(...ext.values);
										return values;
									}, [])
								),
							});
							...
							definitions.push({
								...
								types: (node.types || []).concat(
									extensions.reduce((types, ext) => {
										types.push(...ext.types);
										return types;
									}, [])
								),
							});
...
export function isCollection(node: DefinitionNode): boolean {
	return (
		(node.kind === Kind.OBJECT_TYPE_DEFINITION || node.kind === Kind.UNION_TYPE_DEFINITION) &&
...
export function isComputedField(node: FieldDefinitionNode): boolean {
	return (
		node.directives !== undefined &&
		((node.arguments || []).length > 0 || node.directives!.find(d => d.name.value === 'computed') !== undefined)
	);
}
