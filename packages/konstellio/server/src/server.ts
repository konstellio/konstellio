...
export async function createServer<C extends Context = Context>(
	...
	context.collection = collectionSchemas.reduce(
		(collections, schema) => {
...
export async function loadContext<C extends Context = Context>(
...
export async function loadExtensions(
	...
	const serviceResolvers = await Promise.all(
		(configuration.extensions || []).map(path => resolveModule(path, basedir))
	);
	...
	for (const extension of extensions) {
		loadedExtensions.push({
			typeDefs: extension.typeDefs
				? await callCallable(extension.typeDefs!, configuration, context, loadedExtensions)
				: undefined,
			resolvers: extension.resolvers
				? await callCallable(extension.resolvers!, configuration, context, loadedExtensions)
				: undefined,
			directives: extension.directives
				? await callCallable(extension.directives!, configuration, context, loadedExtensions)
				: undefined,
			main: extension.main,
		});
	}
...
async function loadDrivers(
	configuration: Configuration,
	basedir: string
): Promise<[Database, FileSystem, Cache, MessageQueue]> {
	return Promise.all([
...
export function loadCollectionSchemas(typeDef: DocumentNode): Schema[] {
	return typeDef.definitions.reduce(
		(schemas, node) => {
...
export function loadTypeDefs(loadedExtensions: LoadedExtension[]): DocumentNode {
	const typeDefs = loadedExtensions.reduce(
		(documents, extension) => {
...
export function loadGraphQLSchema(
	...
	const fragments = Object.keys(resolvers).reduce(
		(fragments, typeName) => {
	...
	const extendedSchema = transformSchema(baseSchema, [new ReplaceFieldWithFragment(baseSchema, fragments)]);

	return extendedSchema;
}

export async function loadFastifyInstance(
	configuration: Configuration,
	context: Context,
