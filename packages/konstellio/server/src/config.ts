...
const configurationValidator = Joi.object().keys({
	...
	locales: Joi.object().pattern(/[a-z]{2}/, Joi.string().required()).min(1),
	generate: Joi.object().keys({
		language: Joi.string().allow('typescript').required(),
		destination: Joi.string().required()
	}).default({
		language: 'typescript',
		destination: './src/generated/konstellio'
	}),
	extensions: Joi.array().items(Joi.string()).min(1),
	database: Joi.object({
		driver: Joi.string().required()
	}).pattern(/.*/, Joi.any()).default({
