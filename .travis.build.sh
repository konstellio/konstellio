...
travis_fold start git.checkout
  if [[ ! -d travis-ci/travis-build/.git ]]; then
    travis_cmd git\ clone\ --depth\=50\ --branch\=\'\'\ https://github.com/travis-ci/travis-build.git\ travis-ci/travis-build --assert --echo --retry --timing
    if [[ $? -ne 0 ]]; then
    ...
    travis_cmd git\ -C\ travis-ci/travis-build\ fetch\ origin --assert --echo --retry --timing
    travis_cmd git\ -C\ travis-ci/travis-build\ reset\ --hard --assert --echo
  fi
  rm -f $HOME/.netrc
  travis_cmd cd\ travis-ci/travis-build --echo
...
export TRAVIS_BUILD_DIR=$HOME/build/travis-ci/travis-build
...
export TRAVIS_REPO_SLUG=travis-ci/travis-build
...
travis_fold start nvm.install
  travis_cmd nvm\ install\ 10 --echo --timing
  if [[ $? -ne 0 ]]; then
    echo -e "\033[31;1mFailed to install 10. Remote repository may not be reachable.\033[0m"
    echo -e "Using locally available version 10, if applicable."
    travis_cmd nvm\ use\ 10 --echo
    if [[ $? -ne 0 ]]; then
      echo -e "\033[31;1mUnable to use 10\033[0m"
...
travis_fold start install.1
  travis_cmd npm\ ci --assert --echo --timing
travis_fold end install.1

travis_fold start install.2
  travis_cmd npm\ run\ init --assert --echo --timing
travis_fold end install.2

travis_fold start install.3
  travis_cmd npm\ run\ compile --assert --echo --timing
